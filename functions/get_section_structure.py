...
from functions.src.page import SectionStructure
from functions.src.constants import ENVVAR



@functions_framework.http
def get_section_structure(request):
    """HTTP Cloud Function. 
    ...
    """
    content, status_code = get_request_input(request, ENVVAR.KEY.value)
    if status_code==HTTPStatus.OK:
        try:

            blog = BucketManager.get_structure()
            s = SectionStructure(content)
            s.from_blog_structure(blog)
            content = s.content
